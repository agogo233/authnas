...
ProxyAuth Domains are protected through a collaboration between your reverse-proxy and AuthNas. When a user navigates to a protected domain, your reverse-proxy will check with AuthNas to ensure the user is logged in and should have access. 

You can set up ProxyAuth secured domains on the AuthNas Admin ProxyAuth Domains page.
...
1. Check for a valid user Session cookie in `x-authnas-session`. This is the common way that a user is identified when making the request through the browser.
...
In all denied requests, a `Location` Header containing the location of the AuthNas login page is added to the response. 
...
AuthNas exposes two proxy auth endpoints, which one you use will depend on your reverse-proxy.
...
> [!WARNING]
> You must set up your reverse-proxy **AND** AuthNas correctly to protect a domain! This will usually involve modifying your reverse-proxy config to put the domain 'behind' AuthNas, and then adding that domain to the AuthNas ProxyAuth Domain list with an access group(s).

### Caddy

You can setup ProxyAuth using [Caddy](https://caddyserver.com) as a reverse-proxy with the following CaddyFile which protects domain **app.example.com** using AuthNas hosted on **auth.example.com**
``` Caddy
# Serve AuthNas
auth.example.com {
  reverse_proxy authnas:3000
}

# # or Serve AuthNas on sub-directory of example.com
# example.com {
...
location /api/authz/auth-request {
  ...
  proxy_pass http://authnas:3000/api/authz/auth-request;
}
```
...
> [!WARNING]
> While outside the scope of this guide, you **MUST** setup `https://` termination for your browser-facing reverse-proxy with a certificate that the browser will trust or AuthNas will **NOT** work properly. A bare NGINX reverse-proxy will **NOT** do this for you.
...
http {
  ...
  server {
    ...
    location / {
      include /config/nginx/snippets/proxy.conf;
      proxy_pass http://authnas:3000;
    }
  }

  # # or Serve AuthNas on a sub-directory of example.com
  # server {
  #   listen 443 ssl http2;
  ...
  server {
    ...
    # Snippet make AuthNas available for auth_request
    ...
    location / {
      ...
      # Snippet to send auth_request to AuthNas before accessing protected app
...
4. Fill out the `Advanced` tab; this is done to allow including the `proxy.conf` snippet and will also be required for any protected apps we add later
    ...
    ```
    * <img src="/public/screenshots/NPM_authnas_advanced.png" width="500" />
...
3. Fill out the `SSL` tab with your SSL configuration
4. Fill out the `Advanced` tab; this configuration will include snippets that instruct NGINX to use AuthNas for ProxyAuth
    ``` conf
    include /config/nginx/snippets/auth-location.conf;
...
services:
  traefik:
    ...
    labels:
      ...
      traefik.http.routers.api.middlewares: 'authnas@docker'
    depends_on:
      - authnas
  ...
  authnas: 
    ...
    labels:
      traefik.enable: 'true'
      traefik.http.routers.authnas.rule: 'Host(`auth.example.com`)'
      traefik.http.routers.authnas.entryPoints: 'websecure'
      traefik.http.routers.authnas.tls: 'true'
      traefik.http.middlewares.authnas.forwardAuth.address: 'http://authnas:3000/api/authz/forward-auth'
      traefik.http.middlewares.authnas.forwardAuth.trustForwardHeader: 'true'
      traefik.http.middlewares.authnas.forwardAuth.authResponseHeaders: 'Remote-User,Remote-Name,Remote-Email,Remote-Groups'
    depends_on:
      - authnas-db
  ...
  whoami:
    ...
    labels:
      ...
      traefik.http.routers.whoami.middlewares: 'authnas@docker'
```
