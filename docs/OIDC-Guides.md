...
> [!TIP]
> Placeholders are used for common settings, like `your-client-id`, `your-client-secret`, `your-admin-role`, `https://app-name.example.com`, and `Copy from AuthNas OIDC Info`. OIDC (Endpoint) Info can be found in the dropdown tab on the admin OIDC and OIDC App pages, and is the recommended source of OIDC related Endpoint URLs.
...
> [!NOTE]
> The guides below are maintained by the community and may not always be accurate or current. If you notice any inaccuracies or have suggestions for improvements, please feel free to submit a pull request on the GitHub repository [here](https://github.com/authnas/authnas).

<br>

## <img src="https://cdn.jsdelivr.net/gh/selfhst/icons/svg/actual-budget.svg" width="28" /> Actual Budget

Actual Budget can be set up to use AuthNas as an OIDC Provider in three ways: through Environment Variables, Web GUI, or Config File. See the [Actual Budget OAuth Documentation](https://actualbudget.org/docs/config/oauth-auth/) for full details.

**Environment Variables:**

```bash
ACTUAL_OPENID_DISCOVERY_URL="Copy from AuthNas OIDC Info (OIDC Issuer Endpoint)"
...
Arcane can be set up to use AuthNas as an OIDC Provider in two ways: through the Web GUI or through Environment Variables. See the [Arcane SSO Documentation](https://arcane.ofkm.dev/docs/configuration/sso) for full details.
...
OIDC_ISSUER_URL="Copy from AuthNas OIDC Info (OIDC Issuer Endpoint)"
...
generic OAuth Metadata URL: Copy from AuthNas OIDC Info (Well-Known Endpoint)
generic OAuth Username mapper: preferred_username
generic OAuth Email mapper: email
generic OAuth Login Button: AuthNas
```
...
Client ID: your-client-id
Client Secret: your-client-secret
Auth URL: Copy from AuthNas OIDC Info (Authorization Endpoint)
Token URL: Copy from AuthNas OIDC Info (Token Endpoint)
Fetch user info from: User info URL
User info URL: Copy from AuthNas OIDC Info (UserInfo Endpoint)
```
...
Auth URL: Copy from AuthNas OIDC Info (Authorization Endpoint)
Token URL: Copy from AuthNas OIDC Info (Token Endpoint)
Certificate URL: Copy from AuthNas OIDC Info (JWKs Endpoint)
...
OIDC_CLIENT_ID="your-client-id"
OIDC_CLIENT_SECRET="your-client-secret"
OIDC_ISSUER="Copy from AuthNas OIDC Info (OIDC Issuer Endpoint)"
...
```
Name: AuthNas
Issuer URL: Copy from AuthNas OIDC Info (OIDC Issuer Endpoint)
Client ID: your-client-id
Client Secret: your-client-secret
...
```bash
GRIST_OIDC_IDP_ISSUER="Copy from AuthNas OIDC Info (OIDC Issuer Endpoint)"
...
```
Issuer URL: Copy from AuthNas OIDC Info (Well-Known Endpoint)
Client ID: your-client-id
Client Secret: your-client-secret
...
```
Name: AuthNas
OID Endpoint: Copy from AuthNas OIDC Info (OIDC Issuer Endpoint)
...
Auth Method: Client Secret Post
Client Secret: your-client-secret
Redirect URLs: https://jellyfin.example.com/sso/OID/redirect/AuthNas
```
...
Auth Method: Client Secret Post
Client Secret: your-client-secret
Redirect URLs: https://jellyseerr.example.com/login?provider=authnas&callback=true
```
...
> 3. Login to Komodo via AuthNas OIDC to create a disabled account
...
KOMODO_OIDC_PROVIDER="Copy from AuthNas OIDC Info (OIDC Issuer Endpoint)"
...
Client ID: your-client-id
Client Secret: your-client-secret
Authorization endpoint: Copy from AuthNas OIDC Info (Authorization Endpoint)
Token endpoint: Copy from AuthNas OIDC Info (Token Endpoint)
User endpoint: Copy from AuthNas OIDC Info (UserInfo Endpoint)
...
In the following example, only users in AuthNas with the group `users` or `admins` will be able to log in. Adjust these group names as needed.
...
OPENID_PROVIDER_URL="Copy from AuthNas OIDC Info (Well-Known Endpoint)"
OAUTH_PROVIDER_NAME="AuthNas"
...
> [!NOTE]
> The Redirect URL is displayed after creating AuthNas as an identity provider in Pangolin settings. The callback path may vary if you have multiple OIDC providers configured (e.g., `/auth/idp/2/oidc/callback`).

> [!TIP]
> You can either enable automatic user creation when configuring AuthNas as an identity provider in Pangolin, or manually create users in your Pangolin organization settings using their OpenID Connect ID as the username. The ID format is `XXXXXXXX-XXXX-XXXX-XXXXXXXXXXXX` and can be found in the AuthNas URL when viewing the user profile. Users will display with their email in Pangolin, not the ID.
...
PAPERLESS_SOCIALACCOUNT_PROVIDERS='{"openid_connect": {"OAUTH_PKCE_ENABLED": true, "APPS": [{"provider_id": "authnas","name": "AuthNas","client_id": "your-client-id","secret": "your-client-secret","settings": {"fetch_userinfo": true,"server_url": "https://authnas.example.com/oidc","token_auth_method": "client_secret_basic"}}]}}'
...
Auth Method: Client Secret Basic
Client Secret: your-client-secret
Redirect URLs: https://paperless.example.com/accounts/oidc/authnas/login/callback/
```
...
> [!TIP]
> To link an existing local user to AuthNas:
> - Log in via the AuthNas button on the Paperless login screen
> - When prompted to register, do not proceed
> - Log in with your local user and go to **Profile**
> - Link your local account to AuthNas - the AuthNas email should now appear in connected 3rd party accounts
...
Client ID: your-client-id
Client secret: your-client-secret
Authorization URL: Copy from AuthNas OIDC Info (Authorization Endpoint)
Access token URL: Copy from AuthNas OIDC Info (Token Endpoint)
Resource URL: Copy from AuthNas OIDC Info (UserInfo Endpoint)
...
Proxmox PVE can be set up to use AuthNas as an OIDC Provider in two ways: through Web GUI or Config File. See the [Proxmox PVE Authentication Realms Documentation](https://pve.proxmox.com/wiki/User_Management#pveum_authentication_realms) for full details.
...
# Optional
# Default: (Make AuthNas your default provider)
...
OAUTH_PROVIDER_DOMAIN = "authnas.example.com"
OAUTH_PROVIDER = "authnas.example.com"
OAUTH_AUTHORIZATION_URL = "https://authnas.example.com/oidc/auth"
OAUTH_TOKEN_URL = "https://authnas.example.com/oidc/token"
OAUTH_USER_INFO_URL = "https://authnas.example.com/oidc/me"
...
Client ID: your-client-id
Client Key: your-client-secret
Authority Server: Copy from AuthNas OIDC Info (OIDC Issuer Endpoint)
...
Client ID: your-client-id
Client Secret: your-client-secret
Authorization Endpoint URL: Copy from AuthNas OIDC Info (Authorization Endpoint)
Token Endpoint URL: Copy from AuthNas OIDC Info (Token Endpoint)
User Info Endpoint URL: Copy from AuthNas OIDC Info (UserInfo Endpoint)
Issuer: Copy from AuthNas OIDC Info (OIDC Issuer Endpoint)
Email Claim: email
Display Name Claim: name
Groups Claim: groups
Logout URL: Copy from AuthNas OIDC Info (Logout Endpoint)
```
